  ...
  return (
    <div className="ms:w-full flex-1 flex flex-col items-center text-center z-10 pb-4">
      <motion.h1
        ...
        className={`flex flex-col items-center font-extrabold text-3xl md:text-5xl lg:text-6xl mb-6 ${isChinese ? "tracking-widest" : ""}`}
      >
        <p className={`mb-4 ${isChinese ? "tracking-widest" : ""}`}>{title}</p>
        <AnimationText
          className={`text-2xl w-full md:text-4xl ${isChinese ? "tracking-widest" : ""}`}
      ...
      <motion.div style={{ y: iconTranstion3 }} className="flex flex-wrap justify-center gap-4 items-center">
        <Button
          color="primary"
          radius="md"
          variant="shadow"
        ...
        <Dropdown placement="bottom" trigger="press">
          ...
          <DropdownMenu aria-label="Other platforms">
            ...
            <DropdownItem
              key="firefox"
              startContent={<Firefox size={20} style={{ transform: "scale(0.8)" }}/>}
              onPress={() =>
                window.open(
