...
export default function PricingPage(props: Props) {
  ...
  useEffect(() => {
    window.KODEPAY_APPLICATION_ID =
      process.env.NEXT_PUBLIC_KODEPAY_APPLICATION_ID!; // application_id
    window.KODEPAY_CLIENT_ID = process.env.NEXT_PUBLIC_KODEPAY_CLIENT_ID!; //client_id
    window.KODEPAY_ENV = process.env.NEXT_PUBLIC_KODEPAY_ENV!; //env，development 和  production

    const dom = document.querySelector("kodepay-script");
    if (!dom) {
      const script = document.createElement("script");
      script.src = process.env.NEXT_PUBLIC_KODEPAY_SRC!;
      script.async = true;
  ...
  const getPlan = (plan) => {
    ...
    if (plan.id === "pro") {
      ...
      const origial_data = {
        ...
        payment_method: "card", //"stripe" =>"card","alipay","wechat", k"paypal"=>"paypal"
      ...
      window.KodePay.register_login_information(
        currentUser.user_metadata.user_key
      ).then((res) => {
        console.log("模拟登录", res);
      });
      window.KodePay.open_payment_page_new(price_id, origial_data).then(
        (res) => {
  ...
  return (
    ...
    //             {item.title}
    //           </Title>
    //           {item.badge ? <Badge color="pink">{item.badge}</Badge> : null}
    //         </Group>
