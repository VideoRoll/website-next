  ...
  const handleSubmit = useCallback(
    async (e: any) => {
      ...
      if (redirectCallback) {
        ...
        // 检查是否是登录重定向（通过检查函数名或类型判断）
        const isSigninRedirect = type === 'signin';
        
        if (isDevelopment && isSigninRedirect) {
          // 开发环境，登录成功后跳转到 console 应用的 profile 页面
          window.location.href = `http://localhost:3134/${locale}/profile`;
        } else {
          redirectCallback(locale);
        }
      }
    },
    [captchaToken, onSubmit, locale, redirectCallback, t, turnstile, type]
  );
