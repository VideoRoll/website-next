...
import { Loader2, Download, X, FileVideo, FileText } from 'lucide-react';
...
const COMPRESSION_PRESETS: Record<CompressionPreset, CompressionPresetConfig> = {
  low: { videoBitrate: 1_000_000, audioBitrate: 96_000 },
  medium: { videoBitrate: 2_000_000, audioBitrate: 128_000 },
  high: { videoBitrate: 3_000_000, audioBitrate: 192_000 },
  custom: { videoBitrate: 2_500_000, audioBitrate: 192_000 },
};
...
export default function CompressPage() {
  ...
  const [compressionPreset, setCompressionPreset] = React.useState<CompressionPreset>('medium');
  const [videoBitrate, setVideoBitrate] = React.useState<number>(COMPRESSION_PRESETS.medium.videoBitrate);
  const [audioBitrate, setAudioBitrate] = React.useState<number>(COMPRESSION_PRESETS.medium.audioBitrate);
  ...
  const onCompress = React.useCallback(async () => {
    ...
    addLog('info', `${t('logMessages.compressionStarted')}: ${file.name} -> ${container.toUpperCase()}`);
    ...
    try {
      ...
      const newOutputName = `${baseName}_compressed.${container}`;
      setOutputName(newOutputName);
      addLog('info', `${t('logMessages.compressionCompleted')}: ${newOutputName} (${Math.round(blob.size / 1024)} KB)`);
      
      const compressionRatio = ((1 - blob.size / file.size) * 100).toFixed(1);
      addLog('info', `${t('logMessages.compressionRatio')}: ${compressionRatio}%`);
    } catch (err) {
  ...
  const compressionPresetOptions = React.useMemo(() => [
    { label: t('compressionPresets.low'), value: 'low' as const },
    { label: t('compressionPresets.medium'), value: 'medium' as const },
    { label: t('compressionPresets.high'), value: 'high' as const },
    { label: t('compressionPresets.custom'), value: 'custom' as const },
  ], [t]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {sourceFile ? (
                <div className="w-full border border-border rounded-lg p-3 px-4 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <span className="text-xs text-muted-foreground">({formatFileSize(sourceFile.size)})</span>
                  </div>
                  <button
                    type="button"
                    onClick={(e) => {
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{t('compressSettings')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">{t('compressionPreset')}</label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      {compressionPresetOptions.map((opt) => (
                        <SelectItem key={opt.value} value={opt.value}>
                          {opt.label}
                        </SelectItem>
                      ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={audioBitrate}
                    onChange={(e) => {
                      setAudioBitrate(Number(e.target.value) || 0);
                      setCompressionPreset('custom');
                    }}
                    min={32000}
                    step={16000}
              ...
              <div className="flex gap-2">
                <Button disabled={!canCompress} onClick={onCompress}>
                  {isCompressing ? (
                    <>
                ...
                <Button variant="outline" disabled={!isCompressing} onClick={onCancel}>
                  {t('cancel')}
                </Button>
