...
export function ProfileContent() {
  ...
  const getProviderLabel = () => {
    ...
    if (provider === "google") return t("googleAccount");
    if (provider === "github") return t("githubAccount");
    return t("emailAccount");
  };

  const getProviderMethod = () => {
    const provider = appMeta.provider;
    if (provider === "google") return t("googleOAuth");
    if (provider === "github") return t("githubOAuth");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <h4 className="text-lg font-medium">{t("accountInfo")}</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <p className="text-sm text-muted-foreground">{t("loginEmail")}</p>
            <p className="font-medium break-all">{userEmail}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">{t("loginMethod")}</p>
          ...
          <div>
            <p className="text-sm text-muted-foreground">{t("lastSignInAt")}</p>
